...
public class CassandraDataHandler extends AbstractExpressionDataHandler {
	...
	public void updateEntityInTable(String tableName, DataEntry newProperties)
			throws DataServiceFault {
		PreparedStatement statement = session.prepare(createUpdateEntitySQL(tableName, newProperties));
		BoundStatement boundStatement = bindParams(newProperties, statement, tableName);
		session.execute(boundStatement);
	}
